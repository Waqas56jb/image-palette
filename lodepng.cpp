...
static int WebPMemToUint32(const uint8_t* const ptr) {
  uint32_t A;
...
static int VP8LSubPixels(uint32_t a, uint32_t b) {
...
static int PaletteComponentDistance(uint32_t v) {
  return (v <= 128) ? v : (256 - v);
}

static int PaletteColorDistance(uint32_t col1, uint32_t col2) {
...
// optimize palette
static void optimize_palette(LodePNGColorMode* mode_out, const uint32_t* image, unsigned w, unsigned h, LodePNGPalettePriorityStrategy priority, LodePNGPaletteDirectionStrategy direction, LodePNGPaletteTransparencyStrategy transparency, LodePNGPaletteOrderStrategy order) {
...
static int lodepng_auto_choose_color(LodePNGColorMode* mode_out, const unsigned char* image, unsigned w, unsigned h, const LodePNGColorMode* mode_in){
	...
	n = prof.numcolors;
	grey_ok = !prof.colored && !prof.alpha && n >= 256;
	palettebits = n <= 2 ? 1 : (n <= 4 ? 2 : (n <= 16 ? 4 : 8));
	palette_ok = n <= 256 && prof.bits <= 8;
	if((unsigned long long)w * h < n * 2 || (unsigned long long)w * h < 10) {palette_ok = 0;}
...
extern "C" size_t ZopfliLZ77LazyLauncher(const unsigned char* in, size_t instart, size_t inend, unsigned fs);
...
/* extra filtering */
static unsigned filter(unsigned char* out, unsigned char* in, unsigned w, unsigned h, const LodePNGColorMode* info, LodePNGEncoderSettings* settings){
	...
	if(strategy < LFS_BRUTE_FORCE) {
		...
		// 5 # brute-force		
		if(strategy == LFS_BRUTE_FORCE || strategy == LFS_BRUTE_FORCE2) {
			size_t size[5];
			ucvector attempt[5];
			size_t smallest = 0;
			...
			for(y = 0; y != h; ++y) {
				...
				for(type = 0; type != 5; ++type) {
					...
					if(settings->filter_style < 2 || 1) {
						deflateTune(&stream, 4, 32, 32, 25);
						if (strategy == LFS_BRUTE_FORCE2) {
		...
		// 7 # minsum	
		else if(strategy == LFS_MINSUM_ORG){
			size_t sum[5];
		...
		// 11 # incremental
		else if(strategy == LFS_INCREMENTAL || strategy == LFS_INCREMENTAL2) {
			size_t size[5];
			unsigned char* attempt[5];
			size_t smallest;
			unsigned type, bestType = 0;
			...
			int err = deflateInit2(&dstream, 2, Z_DEFLATED, windowbits(testsize * h), 8, Z_FILTERED);
			if (err != Z_OK) exit(1);
			if(strategy == LFS_INCREMENTAL) {
				deflateTune(&dstream, 4, 32, 32, 25);
			}
			if(strategy == LFS_INCREMENTAL2) {
...
state->error = lodepng_auto_choose_color(&info.color, image, w, h, &state->info_raw);
	if(info.color.colortype == LCT_PALETTE)
{
optimize_palette(&info.color, (uint32_t*)image, w, h, palset.priority, palset.direction, palset.trans, palset.order);
}
...
std::ofstream file(filename.c_str(), std::ios::out|std::ios::binary);
file.write((char*)&header[0], std::streamsize(8));
file.write(buffer.empty() ? 0 : (char*)&buffer[0], std::streamsize(buffer.size()));
}
...
unsigned encode2(std::vector<unsigned char>& out,
...
unsigned char* buffer;
size_t buffersize;
unsigned error = lodepng_encode(&buffer, &buffersize, &in[0], w, h, &state, p);
if(buffer)
{
out.insert(out.end(), &buffer[0], &buffer[buffersize]);
free(buffer);
